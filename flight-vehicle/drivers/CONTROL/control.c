...
//    ctrl.yaw.shell.pid_out = ctrl.yaw.shell.kp * (-Yawtemp2) +ctrl.yaw.shell.ki * ctrl.yaw.shell.increment+ ctrl.yaw.shell.kd * (yaw - yaw_old);		
	...
	ctrl.roll.core.kp_out = ctrl.roll.core.kp * (ctrl.roll.shell.pid_out + sensor.gyro.radian1.x * RtA); 
	ctrl.roll.core.increment += (ctrl.roll.shell.pid_out+ sensor.gyro.radian1.x * RtA ) ;
			//积分限幅
	...
	ctrl.roll.core.kd_out = ctrl.roll.core.kd * (sensor.gyro.radian1.x  - sensor.gyro.histor.x);
	
	ctrl.pitch.core.kp_out = ctrl.pitch.core.kp * (ctrl.pitch.shell.pid_out+ sensor.gyro.radian1.y * RtA ) ;
	ctrl.pitch.core.increment += (ctrl.pitch.shell.pid_out+ sensor.gyro.radian1.y * RtA ) ;
			//积分限幅
	...
	ctrl.pitch.core.kd_out = ctrl.pitch.core.kd * (sensor.gyro.radian1.y - sensor.gyro.histor.y);
	
	ctrl.yaw.core.kp_out = ctrl.yaw.core.kp * (ctrl.yaw.shell.pid_out + sensor.gyro.radian1.z * RtA);
	ctrl.yaw.core.kd_out = ctrl.yaw.core.kd * (sensor.gyro.radian1.z - sensor.gyro.histor.z);
	...
	sensor.gyro.histor.x = sensor.gyro.radian1.x;   //储存陀螺仪（角速度）历史值
	sensor.gyro.histor.y = sensor.gyro.radian1.y;
	sensor.gyro.histor.z = sensor.gyro.radian1.z;
